...
export default function HeroFullscreenVideo() {
  ...
  // État pour le film Hero (chargé depuis la BDD)
  const [trendingMovie, setTrendingMovie] = useState(null);

  // Film par défaut si aucun n'est configuré
  const defaultMovie = {
  ...
  // Charger le film Hero depuis la BDD
  useEffect(() => {
    loadHeroMovie();
  }, []);
  ...
  const loadHeroMovie = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Aucun film configuré, utiliser le film par défaut
        setTrendingMovie(defaultMovie);
      }
    } catch (error) {
      console.error('Erreur lors du chargement du Hero:', error);
      // En cas d'erreur, utiliser le film par défaut
      setTrendingMovie(defaultMovie);
    } finally {
      setLoading(false);
    }
  ...
  // Initialiser le lecteur YouTube une fois que le film est chargé
  useEffect(() => {
    if (!trendingMovie || !trendingMovie.trailerYoutubeId) return;
  ...
  if (loading || !trendingMovie) {
    return (
      <div className="relative h-screen w-full overflow-hidden bg-zinc-950 flex items-center justify-center">
